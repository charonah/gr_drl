...
def evaluate(network, epoch, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        while not done and count < 501:
            ...
            state, reward, done, _, st_reg= env.step(a_in)
            avg_reward += reward
            count += 1
            if reward < -90:
                col += 1
...
class TD3(object):
    ...
    def get_Qvalue(self, action, state):

        action = torch.Tensor(action).unsqueeze(0).to(device)
        state = torch.Tensor(state).unsqueeze(0).to(device)
        current_Q1, current_Q2 = self.critic(state, action)
        current_Q = torch.min(current_Q1, current_Q2)
        return current_Q.cpu().data.numpy().flatten()

    # training cycle----most important
    def train(
        self,
        replay_buffer,
        iterations,
        batch_size=100,
        discount=1,
        tau=0.005,         # tau is how much of base network parameters we will add to target network in soft update
...
test_model = True # test model 
train_model = False # train model  
...
# Create the possiable goal_set & real_goal_index
goal_set = [[4,4], [0,4], [-4,3], [4.5, -2.5],[-4,-2]]
real_goal_index = [0]

env = GazeboEnv("multi_robot_scenario.launch", environment_dim, goal_set)
    ...
    while timestep < max_timesteps:
        ...
        if done:
            if timestep != 0:
                network.train(
                    replay_buffer,
                    episode_timesteps,  # determin training cycles
                    batch_size,
                    discount,
                    tau,
                    policy_noise,                               
            ...
            if timesteps_since_eval >= eval_freq:    #check if it is time to perform an evaluation of the current model
                print("Validating")
                timesteps_since_eval %= eval_freq
                evaluations.append(                  #save model
